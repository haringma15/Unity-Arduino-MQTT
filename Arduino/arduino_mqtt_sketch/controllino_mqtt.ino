...
// Target IP Adress of Server
// TODO remove when done
IPAddress outIp(192, 168, 2, 2);
IPAddress debugIp(192, 168, 2, 110); // Debug Server IP

//Create UDP message object
EthernetUDP Udp;
int incPort  = 26334;     // Incoming Message Port
int destPort = 26000;     // Outgoing Message Port
...
void setup() {
  ...
  IP1 = Ethernet.localIP()[0];
  IP2 = Ethernet.localIP()[1];
  IP3 = Ethernet.localIP()[2];
  IP4 = Ethernet.localIP()[3];
...
void loop() {
  ...
  mqttClient.publish("test/topic", "Hello from Arduino");
...
void reconnect() {
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect("arduinoClient", mqttUser, mqttPassword)) {
      ...
      mqttClient.subscribe("cosa/room3/habitat/solar");
    } else {
